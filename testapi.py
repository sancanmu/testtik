...
class Email:
    def create_email(self):
        return 'terminatorw800@outlook.com'
            #"{}@{}".format("".join(random.choices(string.ascii_lowercase, k = 13)), random.choice(["lasagna.pro", "rblx.rocks", "linustechtips.email"]))
    # def get_mail(self, email):
    #     with requests.Session() as session:base_mail = session.get(f"https://outlook.com/api/inbox/{email}")
    #     if base_mail.json()["emails"] == []:
    #         return None
    #     else:return base_mail.json()["emails"]
...
class Device:
    ...
    devices = [
        {"brand": "samsung",#手机品牌
        "model": ["SM-A127F"],#手机型号，表示具体的手机型号或系列A707F
        "resolution": "1467x720",#屏幕分辨率，表示屏幕的水平和垂直像素数，例如1920x1080。
        "dpi": 300,#屏幕像素密度，表示每英寸的像素数，通常用于衡量屏幕的清晰度。
        "build": ["RP1A.200720.012"],#手机构建版本，表示手机的软件构建版本号，通常用于区分不同的软件版本。
        "rom": ["A127FXXU3AUJ5"],#手机存储空间
        "board": ["universal7884B","mt6768","mt6768","atoll","universal9825","universal7904","universal7904","universal9611","mt6765","msm8953","universal7870","universal9610","universal9610",
                    "universal9611","msm8937","universal7870","universal7870","msm8953","universal7570","msm8953","mt6765","mt6739","sm6150","universal9810","msm8998","universal7904","sdm660",
                    "universal7885","msm8953","universal7885","msm8937","universal7884B","atoll","universal9820","universal3830","mt6739","sdm845","universal9611","universal3830","universal9820",
                    "universal3830","mt6768","sm6150","mt6768","universal9810","universal9611","universal9610","universal7904","kona","msm8937","msm8953","mt6765","msm8937","mt6765","msm8937",
                    "bengal","universal9810","msmnile","universal7904","universal2100_r"],#手机主板
        "core": ["exynos7884B",
        ...
        "device": [
                ...
                {"device": "o1s", "product": "o1sxser"}],#设备名称，表示手机的设备名称或代号
        "display_density": "mdpi",#显示密度，表示屏幕的像素密度，通常以dpi为单位。
        "os": 10,#操作系统，表示手机所使用的操作系统
        }
    ]
...
class Account:
    ...
    def send_code(self, email: str = None, password: str = None) -> None:
        ...
        if not response.json()["message"] == "success":
            print(response.json())
    def verify_code(self, code: int = None, email: str = None, password: str = None) -> None:
        ...
        if response.text.__contains__("session_key"):
            ...
            open("./output/accounts.txt", "a").write(
                f"{self.device[0]}:{self.device[1]}:{email}:{password}:{session_id}\n")
            print(
                f"[ {self.registered} ] Registered Account | [Device ID: {self.device[0]} Install ID:{self.device[1]} Email: {email} Password: {password} Session ID: {session_id}]")
        else:
            print(response.json())
    def register(self) -> None:
        for _ in range(self.accounts_to_register):
            ...
            password = "".join(random.choices(string.ascii_lowercase, k=9)) + "".join(
